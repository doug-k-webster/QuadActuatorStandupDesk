namespace QuadActuatorStandupDesk
{
    ...
    public class PigpiodIf : IDisposable
    {
        ...
        private readonly Dictionary<int, string> errInfo = new Dictionary<int, string>()
        {
        ...
        private readonly List<Callback> gCallBackList = new List<Callback>();

        private readonly List<EvtCallback> geCallBackList = new List<EvtCallback>();
        ...
        public bool CanRead
        {
            get
            {
                if (this.TcpConnection.Stream == null
                    || NotifyTcpConnection.Stream == null)
                    return false;
        ...
        public static int PI_NTFY_FLAGS_BIT(int x)
        {
            return (((x) << 0) & 31);
        }
        ...
        public static int PI_SPI_FLAGS_BITLEN(int x)
        {
            return ((x & 63) << 16);
        }

        public static int PI_SPI_FLAGS_RX_LSB(int x)
        {
            return ((x & 1) << 15);
        }

        public static int PI_SPI_FLAGS_TX_LSB(int x)
        {
            return ((x & 1) << 14);
        }

        public static int PI_SPI_FLAGS_3WREN(int x)
        {
            return ((x & 15) << 10);
        }

        public static int PI_SPI_FLAGS_3WIRE(int x)
        {
            return ((x & 1) << 9);
        }

        public static int PI_SPI_FLAGS_AUX_SPI(int x)
        {
            return ((x & 1) << 8);
        }

        public static int PI_SPI_FLAGS_RESVD(int x)
        {
            return ((x & 7) << 5);
        }

        public static int PI_SPI_FLAGS_CSPOLS(int x)
        {
            return ((x & 7) << 2);
        }

        public static int PI_SPI_FLAGS_MODE(int x)
        {
            return ((x & 3));
        }
        ...
        public int hardware_PWM(UInt32 gpio, UInt32 frequency, UInt32 dutycycle)
        {
            ...
            */
            ...
            exts[0]
                .Contents = UInt32ToBytes(dutycycle);
        ...
        public int wave_add_generic(GpioPulse[] pulses)
        {
            ...
            */
            ...
            exts[0]
                .Contents = UInt32ArrayToBytes(list.ToArray());

            return pigpio_command_ext(PI_CMD_WVAG, 0, 0, exts);
        }

        public int wave_add_serial(UInt32 user_gpio, UInt32 baud, UInt32 databits, UInt32 stophalfbits, UInt32 offset, byte[] str)
        {
            GpioExtent[] exts = new GpioExtent[]
            {
                new GpioExtent(),
            ...
            */
            ...
            exts[0]
                .Contents = UInt32ArrayToBytes(array);
        ...
        public int run_script(UInt32 script_id, UInt32[] paramArray)
        {
            ...
            */
            ...
            exts[0]
                .Contents = UInt32ArrayToBytes(paramArray);
        ...
        public int i2c_open(UInt32 i2c_bus, UInt32 i2c_addr, UInt32 i2c_flags)
        {
            ...
            */
            ...
            exts[0]
                .Contents = UInt32ToBytes(i2c_flags);
        ...
        public int bb_spi_open(UInt32 CS, UInt32 MISO, UInt32 MOSI, UInt32 SCLK, UInt32 baud, UInt32 spiFlags)
        {
            GpioExtent[] exts = new GpioExtent[]
            {
                new GpioExtent()
        ...
        int custom_1(UInt32 arg1, UInt32 arg2, string argx, byte[] retBuf)
        {
            ...
            */
            ...
            exts[0]
                .Contents = System.Text.Encoding.UTF8.GetBytes(argx);

            return pigpio_command_ext(PI_CMD_CF2, (int) arg1, (int) arg2, exts);
        }
        ...
        int custom_2(UInt32 arg1, string argx, byte[] retBuf)
        {
            ...
            */
            ...
            lock (LockObject)
            {
                bytes = pigpio_command_ext(PI_CMD_CF2, (int) arg1, retBuf.Length, exts);
        ...
        int shell_(string scriptName, string scriptString)
        {
            ...
            */
            ...
            exts[0]
                .Contents = System.Text.Encoding.UTF8.GetBytes(scriptName + "\0"); /* include null byte */

            exts[1]
                .Contents = System.Text.Encoding.UTF8.GetBytes(scriptString);

            return pigpio_command_ext(PI_CMD_SHELL, scriptName.Length, 0, exts);
        }
        ...
        int file_open(string file, UInt32 mode)
        {
            ...
            */
            ...
            return pigpio_command_ext(PI_CMD_FO, (int) mode, 0, exts);
        }
        ...
        int file_write(UInt32 handle, byte[] buf)
        {
            ...
            */
            ...
            return pigpio_command_ext(PI_CMD_FW, (int) handle, 0, exts);
        }
        ...
        int file_read(UInt32 handle, byte[] buf)
        {
            ...
            lock (LockObject)
            {
                bytes = pigpio_command(PI_CMD_FR, (int) handle, buf.Length);
        ...
        int file_seek(UInt32 handle, UInt32 seekOffset, int seekFrom)
        {
            ...
            */
            ...
            exts[0]
                .Contents = UInt32ToBytes((UInt32) seekFrom);

            return pigpio_command_ext(PI_CMD_FS, (int) handle, (int) seekOffset, exts);
        }
        ...
        int file_list(string fpat, byte[] buf)
        {
            ...
            */
            ...
            exts[0]
                .Contents = System.Text.Encoding.UTF8.GetBytes(fpat);
        ...
        int wait_for_edge(int pi, UInt32 user_gpio, UInt32 edge, double timeout)
        {
            ...
            var id = callback(
                user_gpio,
                edge,
                (gpio, level, tick, user) =>
                {
                    triggered = 1;
                });

            while (triggered == 0
                && (time_time() < due)) time_sleep(0.05);
        ...
        private int pigpio_notify()
        {
            if (NotifyTcpConnection == null
                || NotifyTcpConnection.Stream == null
                || NotifyTcpConnection.Stream.CanWrite == false
                || NotifyTcpConnection.Stream.CanRead == false)
            {
                return (int) EError.pigif_unconnected_pi;
            }
        ...
        private int pigpio_command_ext(int command, int p1, int p2, GpioExtent[] exts)
        {
            ...
            int index = 0;
            cmdBytes.CopyTo(bytes, index);
            index += cmdBytes.Length;
            foreach (var ext in exts)
            {
                ext.Contents.CopyTo(bytes, index);
                index += ext.Contents.Length;
            }
        ...
        private int recvMax(byte[] buf, int sent)
        {
            ...
            */
            ...
            if (sent < buf.Length) count = sent;
            else count = buf.Length;
